...
let business={
  name:'Nombre profesional',nif:'',email:'',address:'',phone:'',iban:'',invoiceSeries:'FAC',activity:'',
  ...(JSON.parse(localStorage.getItem('autonomiaBusiness')||'null')||{})
};
...
function nextInvoiceNumber(){
  const series=(business.invoiceSeries||'FAC').trim().toUpperCase()||'FAC';
  return `${series}-${new Date().getFullYear()}-${String(invoices.length+1).padStart(3,'0')}`;
}
...
document.getElementById('previewInvoice').addEventListener('click',()=>{
  ...
  const count=Number(document.getElementById('invoiceSessions').value||0);
  const rate=Number(document.getElementById('invoiceRate').value||0);
  document.getElementById('previewNumber').textContent=nextInvoiceNumber();
  ...
  document.getElementById('previewNif').textContent=business.nif?`NIF/CIF: ${business.nif}`:'';
  ...
  document.getElementById('previewIban').textContent=business.iban?`Forma de pago · IBAN: ${business.iban}`:'';
...
document.getElementById('saveBusiness').addEventListener('click',()=>{
  business={
    name:document.getElementById('businessName').value.trim()||'Nombre profesional',
    nif:document.getElementById('businessNif').value.trim(),
    ...
    iban:document.getElementById('businessIban').value.trim().toUpperCase(),
    invoiceSeries:document.getElementById('businessInvoiceSeries').value.trim().toUpperCase()||'FAC',
    activity:document.getElementById('businessActivity').value.trim()
  };
  saveAll();
  document.getElementById('businessStatus').textContent='Datos guardados correctamente.';
});
['Name','Nif','Email','Address','Phone','Iban','InvoiceSeries','Activity'].forEach(key=>{
  document.getElementById(`business${key}`).value=business[key.charAt(0).toLowerCase()+key.slice(1)]||'';
});
